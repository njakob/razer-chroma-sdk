...
namespace ChromaSDK {
  ...
  namespace Keyboard {
    ...
    typedef struct CUSTOM_KEY_EFFECT_TYPE {
      COLORREF Color[MAX_ROW][MAX_COLUMN]; //!< Grid layout. 6 rows by 22 columns.
      COLORREF
      Key[MAX_ROW][MAX_COLUMN]; //!< Keys information. 6 rows by 22 columns.
                                //!< To indidate there is a key effect, OR with
                                //!< 0x01000000. i.e. Key[0][1] = 0x01000000 | Color;
  ...
  namespace Mousepad {
    ...
    typedef struct CUSTOM_EFFECT_TYPE {
      RZCOLOR Color[MAX_LEDS]; //!< An array of colors for all the sides of the
                               //!< mousepad. First LED starts from top-right corner.
  ...
  namespace Keypad {
    ...
    typedef struct CUSTOM_EFFECT_TYPE {
      RZCOLOR Color[MAX_ROW][MAX_COLUMN]; //!< Custom effect.
                                          ...
                                          //!< Color[0] is valid. Use index '0'
                                          //!< to change the keypad color.
    } CUSTOM_EFFECT_TYPE;
